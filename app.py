...
# ==========================================
# 3. CATEGORY & SIGNAL DATA ARCHITECTURE
# ==========================================
...
CATEGORY_SIGNALS_FALLBACK = {
    "🛒 E-Commerce & Viral Shopping": [
        "Ergonomic Desk Accessories", "Smart Pet Grooming Hardware", "Self-Cleaning Water Bottles", "MagSafe Powerbanks"
    ],
    "💻 Digital Products & AI Tools": [
        "Automated AI Workflow Libraries", "Micro-SaaS Invoicing Plugins", "Digital Finance Dashboards", "Generative Video Editing Tools"
    ],
    "✈️ Travel, Hotels & Food": [
        "Hidden Hill Station Stays", "Regional Fusion Food Spots", "Cloud Kitchen Collaborations"
    ],
    "🏛️ Politics, News & Civic Events": [
        "Assembly Election Rallies", "Digital Economy Debates", "Infrastructure Policy Protests"
    ],
    "🛕 Faith, Festivals & Sacred Travel": [
        "Ancient Temples Circuit Travel", "Festival Handicrafts & Decor", "Pilgrimage Eco-Resorts"
    ]
}
...
def create_pdf_blueprint(asset_name, category, role, viral_score, window, result):
    ...
    story = [
        ...
        Paragraph(f"<b>Asset:</b> {clean_asset} | <b>Category:</b> {category} | <b>Role:</b> {role}", body_style),
        Paragraph(f"<b>Predictive Score:</b> {viral_score} | <b>Window:</b> {window}", body_style),
        Spacer(1, 10)
    ]
...
def generate_master_intelligence(keyword_asset, category, target_role, platform, timeframe, velocity_score):
    clean_asset = sanitize_trend_input(keyword_asset)

    default_response = {
        "viral_score": f"{velocity_score}%",
        "prediction_window": f"Active Lifecycle ({timeframe})",
        "profit_model": f"• **Primary Model:** Direct monetization for {target_role}.\n• **Funnel:** Convert traffic for '{clean_asset}' via optimized landing touchpoints.",
        "content_directives": f"• **Angle:** Targeted messaging for {target_role}.\n• **Key Points:** 1. Demand spike 2. Social proof 3. Direct CTA.\n• **Visuals:** Dynamic overlays & fast cuts.",
        "execution_hook": f"• **0-3s Cue:** Demonstration/green-screen of '{clean_asset}'.\n• **Text Overlay:** \"Stop scrolling! Watch this...\"\n• **Script:** \"Here is why {clean_asset} is currently trending...\"",
        "ad_copy": f"Unlock the full potential of {clean_asset}! 🚀\n\nComment 'ACCESS' for details.\n\n#{clean_asset.replace(' ', '')} #TrendPulse",
        "action_blueprint": "1. HOUR 1: Prepare creative hook.\n2. HOUR 6: Launch test campaign.\n3. HOUR 48: Optimize based on engagement.",
        "competitor_intelligence": "• **Focus:** Short, punchy video hooks.\n• **Duration:** 12-25 seconds.\n• **Benchmark:** High share-to-view ratio.",
...
• 00-03s (Hook): Extreme close-up of {clean_asset} solving a core problem. Text Overlay: "Stop scrolling!" | Audio: "If you need a better solution for {clean_asset}, look at this."
• 03-08s (Proof): Dynamic demonstration of key features. Text Overlay: "Top Rated Feature ✨" | Audio: "This automatically solves your biggest daily issue."
• 08-15s (CTA): Point to screen with animated offer box. Text Overlay: "Comment 'WANT' for 15% OFF 👇" | Audio: "Tap the link below or comment 'WANT' for an instant discount link!"

2. AI TEXT-TO-VIDEO GENERATOR PROMPT (Runway / CapCut / Sora)
"A fast-paced 9:16 vertical video showcasing {clean_asset}. Modern cinematic lighting, 4K resolution, authentic UGC feel, high contrast, clean backdrop."

3. CAPTION & HASHTAGS
"Transform your setup with {clean_asset}! 🚀 Comment 'WANT' below for direct access.\n#{clean_asset.replace(' ', '')} #ViralFinds #Trends"
"""
    }
    ...
    try:
        ...
        prompt = f"""
Generate a complete JSON strategy blueprint and video production prompt for:
- Asset: "{clean_asset}"
- Category: "{category}"
- Target Role: "{target_role}"
- Platform: "{platform}"
...
{{
  ...
  "profit_model": "Monetization model tailored to {target_role}",
  "content_directives": "• **Angle:** ...\\n• **Key Points:** 1... 2...",
  "execution_hook": "• **0-3s Cue:** ...\\n• **Text:** ...\\n• **Script:** ...",
  "ad_copy": "Caption and Call to Action",
  "action_blueprint": "1. HOUR 1...\\n2. HOUR 6...\\n3. HOUR 48...",
  "competitor_intelligence": "• **Focus:** ...\\n• **Duration:** ...",
  "production_prompt": "A complete, copy-paste ready prompt containing:\\n1. 15-second Storyboard (0-3s, 3-8s, 8-15s with Visuals, Text Overlay, Audio Script)\\n2. AI Video Generator Prompt (for Runway/CapCut/Sora)\\n3. Final Caption & Hashtags"
}}
"""
...
# Fetch Signal Telemetry
active_signals = fetch_filtered_radar_signals(geo_map[geo_option], platform_source, selected_category, timeframe)
...
with right_col:
    with st.container(border=True):
        ...
        if st.button("⚡ Generate Strategy Blueprint", use_container_width=True):
            with st.spinner("Analyzing commercial telemetry & generating blueprint..."):
                st.session_state["active_blueprint"] = generate_master_intelligence(
                    selected_asset, selected_category, target_role,
    ...
    with st.container(border=True):
        ...
        if not bp:
            ...
            pdf_buf = create_pdf_blueprint(
                ...
                selected_category, st.session_state.get("active_role", target_role),
